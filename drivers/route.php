...
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	$json = json_decode($obj->getRideDetails('route', $id, driver, ''));
	$response = $json[0];
}
?>
				...
				<div class="card card-box">
					...
					<div class="card-body ">
						<form class="form-inline mg-bottom-10" action="#">
							<input type="button" id="journey_start" class="btn deepPink-bgcolor" value="Start Journey" />
							&nbsp;<h4 class="display-1" id="sw-time">00:00:00</h4>
						</form>
		...
		<div class="row">
			<div class="col-md-6">
				<div class="card card-box">
					...
					<div class="card-body">
						<button type="button" id="accept" class="btn deepPink-bgcolor">Accept Request</button>
						<input type="hidden" id="drv" value="<?=driver?>">
						<input type="hidden" id="tid" value="<?=base64_decode($id)?>">
						<div class="tab-content">
							<div class="row">
								...
								<!--.col-md-3-->
								<div class="col-md-8" id="trip_id"><?= $response->trip_id ?></div>
								<!--.col-md-9-->
							</div>
							<!--.row-->
							<div class="row">
							...
							<div class="row">
								<div class="col-md-3">Passenger Bid</div>
								<!--.col-md-3-->
								<div class="col-md-1">:</div>
								<!--.col-md-3-->
								<div class="col-md-8"><?php if(!empty($response->passenger_bid)){echo $response->passenger_bid;}else{echo "no bids";} ?></div>
								<!--.col-md-9-->
							</div>
							<!--.row-->
						</div>
...
<script src="https://maps.googleapis.com/maps/api/js?key=<?php echo API_KEY ?>&libraries=places&callback=<?php if (pathinfo(__FILE__, PATHINFO_FILENAME) == 'route') {
	...
	<script type="text/javascript">
					...
					if (checkOnlineStat()) {
						response.forEach(function(argument) {
							if (!('Notification' in window)) {
								...
								var options = {
									body: "you've got a new ride request",
									tag: 'preset',
		...
		setTimeout(getNewRideRequests, 5000);
		var checkOnlineStat = () => {
	...
	<script>
		...
		init: function() {
			...
			sw.ego = document.getElementById("journey_start");
		...
		start: function() {
			...
			$.ajax({
				...
				dataType: "json",
				data: {
					startRide: '<?= $response->trip_id ?>'
				},
				success: function(response) {
					$.toast({
						heading: "Notification",
						position: 'top-right',
						text: 'You have started this journey',
						hideAfter: false,
						icon: 'info'
					});
				}
		...
		stop: function() {
			...
			sw.timer = null;
			sw.ego.value = "Start Journey";
			sw.ego.removeEventListener("click", sw.stop);
			sw.ego.addEventListener("click", sw.start);
			var stopTime = sw.etime.innerHTML;
			...
			$.ajax({
				...
				data: {
					endRide: stopTime,
					rid: '<?= $response->trip_id ?>'
				},
				success: function(response) {
					$.toast({
						heading: "Notification",
						position: 'top-right',
						text: 'You have ended this journey',
						hideAfter: false,
						icon: 'info'
					});
					getDriverExperience();
	...
	var getDriverExperience = () => {
		...
		swal({
			title: "<h4>What did you think of the ride?</h4>",
			text: "<div id='jRate'></div><br><div class='form-group'><textarea class='form-control' id='comment' rows='3'></textarea></div>",
			html: true,
			type: "info",
			...
			$.ajax({
				...
				data: {
					drvStarRating: stars,
					comment: comment,
					tid: '<?= $response->trip_id ?>'
				},
				dataType: "text",
				success: function(response) {
					setTimeout(function() {
						swal('Alright',
							"Your review has been submitted", 'success');
					}, 2000);
				}
			});
