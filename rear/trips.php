					...
					<div class="card-body ">
						<div class="table-scrollable">
							<table id="tableExport" class="display">
								...
								<tbody>
									<?php $sl = $obj->generalSelectStatement('select * from '.rd.' order by date desc');
									$res = $sl->_general_result; 	
									if (!empty($res)) {
										$num = 1;
										foreach ($res as $key => $value) {
											$driver = json_decode($obj->getRideDetails('driverDetails',$value->driver_id,'',drv));
											?>
											<tr>
												<td><?=$num?></td>
												<td><?=$value->trip_id?></td>
												<td>
													<?php echo $driver[0]->firstname.' '.$driver[0]->lastname; ?>
												</td>
												<td>
													<?php
													$user = json_decode($obj->getRideDetails('passDetails',$value->customer_id,'',mt));
													echo $user[0]->first_name.' '.$user[0]->lastname;
													?>
												</td>
												<td><?=$value->location?></td>
												<td><?=$value->destination?></td>
												<td><span
													class="label label-sm box-shadow-1 label-<?php if($value->location == 'canceled'){echo 'danger';}else{echo 'success';}?>"><?=$value->status?></span></td>
													<td><?=$value->pickup_time?></td>
													<td>
														<a href="route.php?tid=<?=base64_encode($value->trip_id)?>" class="btn btn-tbl-delete btn-xs">
